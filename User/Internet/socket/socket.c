...
/**
 * @brief 初始化指定的Socket通道
 *
 * 该函数用于初始化W5500芯片的特定Socket通道，设置通信协议、端口号等参数。
 *
 * @param s Socket通道号(0-7)
 * @param protocol 通信协议类型(TCP, UDP, IPRAW, MACRAW, PPPOE)
 * @param port 本地端口号
 * @param flag 附加标志
 * @return uint8 初始化成功返回1，失败返回0
 */
uint8 socket(SOCKET s, uint8 protocol, uint16 port, uint8 flag)
{
    ...
    if (
        ...
        close(s);  // 关闭之前可能打开的Socket

        // 设置Socket的模式寄存器(Sn_MR)
